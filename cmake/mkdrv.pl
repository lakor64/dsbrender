...
sub run_h2inc {
    my ($inpath, $outfile, $tmpdir, @includes) = @_;
    ...
    my @params = (
        ...
        '-DBASED_FLOAT=1', '-D_WIN32', '-D__WIN_32__',
...
if (scalar @leftovers != 3) {
    die "Usage: $0 [/Idir ...] <infile> <outfile> <tmpdir>\n";
}

my ($infile, $outfile, $tmpdir) = @leftovers;

run_h2inc($infile, $outfile, $tmpdir, @includes);
